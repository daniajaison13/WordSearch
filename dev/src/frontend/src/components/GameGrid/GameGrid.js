...
const GameGrid = ({
		...
		if (words.includes(word) || words.includes(reversedWord)) {
			...
			if (level === 1) {
				...
				setTimeout(() => {
					...
					setCanContinue(true);
					handleClear(); // Clear selected blocks only when the word is wrong
				}, 2800);
			} else {
				setTimeout(() => {
					setOpenedLetters(prevOpenedLetters);
	...
	const endGame = async () => {
		...
		if (token === null) {
			await apiInstance
				.post(apiUrl + `/game/finish/${gameHistoryId}`, data)
				.then((res) => {
	...
	const shareScore = async (mode) => {
		...
		if (token === null) {
			await apiInstance
				.post(apiUrl + `/leaderboard/sharescore/${gameHistoryId}`, data)
				.then((res) => {
					if (res.data.status) {
